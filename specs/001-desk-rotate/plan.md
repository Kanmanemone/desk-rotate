...
정해진 간격마다 사용자가 지정한 데스크톱 범위(시작~끝, 1-based) 안에서 Windows 가상 데스크톱을 자동으로 순환 전환하고, 범위 안 데스크톱마다 항상-위 테두리 없는 플로팅 창을 통해 남은 시간을 보여주는 개인용 Windows 유틸리티. 창은 기본적으로 남은 시간 숫자만 보이는 최소 보기로 시작하며, 클릭하면 총 예상 실행 시간·데스크톱별 전환 횟수 등을 보여주는 상세 보기로 전환된다. 전환은 `SendInput`으로 시뮬레이션한 `Ctrl+Win+←/→` 키 입력으로 수행하고, 공식 문서화된 `IVirtualDesktopManager.IsWindowOnCurrentVirtualDesktop`으로 각 전환 결과를 검증해 실패 시 재시도·자가 보정한다(비공식 COM 인터페이스나 포커스 강탈 트릭은 쓰지 않음). 플로팅 창을 닫으려 하면 종료 확인 절차를 거치며, 확정 시에만 프로그램 전체가 종료된다.
